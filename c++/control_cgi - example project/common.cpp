...
// Constructor
Stopwatch2::Stopwatch2() { T0 = peripherals.bio.usTime(); }


// Reset
void Stopwatch2::reset() { T0 = peripherals.bio.usTime(); }


// Get Elapsed Time
// out: returns seconds since the last call to reset() or this class' constructor
double Stopwatch2::elapsed() { return  (peripherals.bio.usTime() - T0) * 1e-6; }


bool Stopwatch2::hasElapsed(uint32_t us) { return  peripherals.bio.usTime() - T0 >= us; }
...
CgiRequest::CgiRequest() {
    urlEnc = nullptr;
    urlEncSz = 0;
    contentLength = contentType = queryString = remoteAddr = requestMethod = serverAddr = nullptr;
    serverAddr2[0] = 0;
    contentLen = 0;
}
...
// Initialize
// If not already initialized, get key environment variables and read any POST data from stdin.
void CgiRequest::init() {
    if (urlEnc)  return;    // already initialized?

    urlEnc = new char[MAX_URLENC_SIZE + 1];
    urlEncSz = 0;

    contentLength = getEnv("CONTENT_LENGTH");
    if (sscanf(contentLength, "%u", &contentLen) != 1)  contentLen = 0;
    contentType   = getEnv("CONTENT_TYPE");
    queryString   = getEnv("QUERY_STRING");
    remoteAddr    = getEnv("REMOTE_ADDR");
    requestMethod = getEnv("REQUEST_METHOD");
    serverAddr    = getEnv("SERVER_ADDR");
    if (strEq(serverAddr, "192.168.2.99"))  strcpy(serverAddr2, "192.168.2.99");
    else {
        FILE *src = fopen("/etc/hostname", "r");
        if (src != nullptr) {
            if (fgets(serverAddr2, sizeof serverAddr2, src) == nullptr)  serverAddr2[0] = 0;
            strRTrim(serverAddr2);
            fclose(src);
        }
    }

    if (strEq(requestMethod, "GET")) {
        size_t n = strlen(queryString);
        if (n > MAX_URLENC_SIZE)  throwException("Too Much GET Data");
        memcpy(urlEnc, queryString, n);
        urlEncSz = n;
    }
    else if (strEq(requestMethod, "POST")) {
        if (!strEq(contentType, "application/x-www-form-urlencoded"))  throwException("HTTP Content-Type is not application/x-www-form-urlencoded");
        size_t n = fread(urlEnc, 1, MAX_URLENC_SIZE + 1, stdin);
        if (n > MAX_URLENC_SIZE)  throwException("Too Much POST Data");
        urlEncSz = n;
    }
    else  throwException("HTTP Request Method is neither GET nor POST");
    urlEnc[urlEncSz] = 0;
}


// Get Key's String Value from URL-Encoded String
// in: key = key's name
//     bufSz = buf's size in bytes (>=1)
// out: buf = key's URL-encoded value as a NUL-terminated string, or "" if key not found; values will be truncated to fit the buffer
//      returns true if success, else key not found
bool CgiRequest::get(const char *key, char *buf, size_t bufSz) const {
    if (urlEnc != nullptr) {
        const char *p = urlEnc;
        ...
        while (*p != 0) {
            if (memcmp(p, key, keyLen) == 0 && p[keyLen] == '=') {
                ...
                size_t n  =  q == nullptr  ?  strlen(p)  :  q - p;
                if (n > bufSz - 1)  n = bufSz - 1;   // truncate value
...
// Get Key's int Value from URL-Encoded String
// in: key = key's name
// out: value = value, or 0 if any error (key not found, syntax error, or value out of range)
//      returns true if success, else false
bool CgiRequest::get(const char *key, int &value) const {
    char s[20];
    if (get(key, s, sizeof s) && strToInt(s, value))  return true;
    value = 0;
    return false;
}


// Get Key's double Value from URL-Encoded String
// in: key = key's name
// out: value = value, or 0.0 if any error (key not found, syntax error, or value out of range)
//      returns true if success, else false
bool CgiRequest::get(const char *key, double &value) const {
    char s[20];
    if (get(key, s, sizeof s) && strToDbl(s, value))  return true;
    value = 0.0;
    return false;
}
